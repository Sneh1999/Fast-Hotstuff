...
for i in range(int(5)):
    ...
    if i == 0:
        ...
        run_cmd = f"docker run -d --name {name} --net node-network --expose={ports[i]} -p {ports[i]}:{ports[i]} --env PRIVATE_KEY={private_keys[i]} --env PUBLIC_KEYS={s.join(public_keys)} --env FLASK_RUN_PORT={ports[i]}  --env PORTS={s.join(ports)} --env NODE_ID={i}  --env PRIMARY=False hotstuff-node"
    print(run_cmd)
    os.system(run_cmd)
